...
|---|---|---|
| Movement (CKZ physics) | 🟡 PARTIAL | Core-owned detours dispatched per-mode via `IKzMovementMode`. Prestrafe, perf/air-cap, slopefix + commit-gate, bump-segment re-prediction value-exact. **SlopeFix/perf timing base fixed** (landing uses the pre-clip air velocity with fall-Z intact; perf takeoffTime = curtime−frametime). **Half-tick input quantization ported** (RunCommand pre-hook snaps CKZ subtick When to {0,0.5}, cs2kz OnSetupMove). Remaining: the jump-latch re-arm (movement-service field absent from the NuGet Sharp.Shared the satellite builds against) + the forced-half-tick-subtick INJECTION (m_arrForceSubtickMoveWhen unexposed). Water cap is already covered by the global GetMaxSpeed override (250+prestrafe, every movement type incl. water). Feel is now cs2kz-close; a live A/B is the final check. |
...
| Styles | 🟡 PARTIAL | ABH/LGJ faithful, timer-stops on change. **Style-incompatibility check added** (ad+ws refused — no frozen-WASD). AutoUnduck (cs2kz's 3rd real style) needs m_flLastDuckTime/m_bDucked/button-state — the same movement-service internals the NuGet Sharp.Shared satellites build against don't expose (cf. force_unduck); blocked there. Ice/LowGrav/WSOnly/ADOnly are our own extras. |
| Timer | 🟡 PARTIAL | Timer engine + PRO/STANDARD + course switching. Start-gate has JustLanded + JustTeleported (0.05s each). Checkpoints purge on start-zone/disconnect. **End-zone missed-checkpoint gate added** (can't finish a run that skipped checkpoint zones — timer keeps running). Remaining: true pause-freeze. |
...
| HUD | 🟡 PARTIAL | One HTML block, **now `h:mm:ss.mmm` formatted** (hours shown past the hour). Remaining: perf-color tint, compact mode, particle MHUD — cosmetic polish vs cs2kz's HUD. |
...
| Goto / Fov / Measure / Pistol / Noclip | 🟡 PARTIAL | Goto (timer-block done), Fov, Pistol (**strip-enforce done** — replaces not stacks), Noclip (**respawn safeguard done**), Measure (**eye aim-trace done** — measures where you look). Remaining: Measure's 2 extra subcommands + Pistol team-awareness (minor). |
