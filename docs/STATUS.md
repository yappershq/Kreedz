...
| Modes | 🟡 PARTIAL | Registry + both 33-cvar tables faithful. **CanTouchTimerZone ported for both modes** (VNL: full ticks; CKZ: full+half — subtick-time zone touches rejected via the mode hook, gated in ZoneModule). VNL's triggerfix duties live in Core's TriggerModifierModule (per-tick overlap scan + path sweep, all modes). Mode/style switch now invalidates the run. OnTeleport movedata sync: closed by architecture — cs2kz patches live MoveData because its trigger teleports fire mid-physics; ours execute in the ProcessMove pre-hook (before the engine reads state) or post-tick, so the engine always starts from the teleported pawn and there is no stale movedata to sync. Bump-segment replay done (TryPlayerMove re-prediction in Core, all modes); knife-out done (slot3 via PlayerModeChanged). Per-mode jumpstat tiers not wired. |
...
| Courses | 🟡 PARTIAL | **Course-switch-on-start-zone-touch works** (touching a foreign course's start zone stops the run and moves you onto that course, cs2kz behavior) and courses announce their **mapping-API names** (timer_course_name; Main/Bonus N fallback). **Mode is now part of the record key** (KzModes index stamped on save; every save-path PB/SR comparison filters by it — a VNL run can never compare against or overwrite a CKZ record; LiteDB reads old rows as Mode=CKZ). WR/PB announce comparisons + cache reads are mode-filtered too (GetRecords/GetRecord mode overloads) — the record pipeline is mode-keyed end to end for the save/announce flow; !top/!btop listings filter by the caller's mode — the record pipeline is mode-keyed through save, announce, PB, and listings. |
...
| Options / Preferences | 🟡 PARTIAL | DB-persisted per-player store, now published cross-plugin (IKzPreferences) so external plugins read options too. Wired keys: mode, fov, styles, hide, beam, jsFailstats, jsAlways (toggle commands). Missing: cs2kz's full 65-key set (each lands as its feature ports), typed accessors, local/global merge, server-config option layer. |
| Quiet (!hide) | 🟡 PARTIAL | Per-viewer transmit hiding of other players + carried weapons (controller hooks + per-viewer state), preference-persisted, reapplied on spawn/pref-load. **Sounds suppressed too** (PostEventAbstract receiver filtering on FireBullets/WeaponSound/SosStartSound) — hidden players are invisible AND inaudible, like cs2kz. Remaining: nothing (custom-particle filtering n/a — we ship no custom particles). |
| Beam (!beam) | 🟡 PARTIAL | Airborne trail: reused env_beam segment ring per player (~1.5s at 64t), preference-persisted, teleport-artifact length guard. Deviation: cs2kz renders via its own shipped particle file with feet/ground styles + offset pref — we have no custom particle addon, so one beam style. |
| Paint (!paint) | 🟡 PARTIAL | **Standalone plugin (Kreedz.Paint)** — installable/removable independently (consumes Core's published IKzCommands/IKzPreferences, degrades gracefully without them). Takeoff X-marks from a reused env_beam ring (green=perf, red=non-perf), preference-persisted — the practical bhop-line-reading feature. **Real engine decals ACTIVE** (boot-verified: UTIL_DecalTrace sig + tier0 _MakeGlobalSymbol both resolve; ground-trace → 192-byte native trace buffer → DecalTrace, GE_PlaceDecalEvent recolored green/red while pending — cs2kz's exact mechanism), with automatic env_beam X-mark fallback if a game update breaks either sig. Pending: in-game visual confirmation of the rendered decal; per-viewer opt-out via TransmitManager WorldDecals when wanted. |
| Rank titles (!rank) | 🟡 PARTIAL | **Standalone plugin (Kreedz.RankTitles)** — same satellite pattern. cs2kz's title ladder (Beginner→Legend) over the LOCAL points percentile (GetPlayerPointsRank). Swaps to global percentiles when a KZGlobalteam key/backend lands. |
| Ztopwatch (!zt) | 🟡 PARTIAL | Personal practice stopwatch between two user-placed corner-defined AABB zones (env_beam edges, green/red), start re-armed per tick inside / on-takeoff with !zt jump, stop-on-land toggle, main-timer + CanTouchTimerZone gates. Missing: cs2kz's per-point status readout + particle-edge visuals. |
| Spec (!spec) | 🟡 PARTIAL | Spectate-by-name: run stop, slay-before-team-switch (ghost-pawn guard), spectator join, first-person observer lock next frame; bare !spec = free roam. Missing: spectator-list HUD line, cs2kz's pause-aware CanSpectate (needs real pause). |
