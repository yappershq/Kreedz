...
|---|---|---|
| Movement (CKZ physics) | 🟡 PARTIAL | Core-owned detours (AirMove/Categorize/TryPlayerMove/AirAccelerate + PlayerProcessMovePost) dispatched per-mode via `IKzMovementMode`. Prestrafe (wishdir-vs-velocity), perf/air-cap, TryPlayerMove slopefix + commit-gate, bump-segment TryPlayerMove re-prediction all live and value-exact. **Actively porting (this session):** CKZ half-tick input quantization (OnSetupMove when-snap + forced half-tick subtick moves) and the SlopeFix/perf landing-velocity timing base — the last feel-level internals. Beyond these: water-speed cap + the rarely-hit CGameMovement tail. |
...
| Styles | 🟡 PARTIAL | ABH/LGJ faithful, timer-stops on change. Remaining: AutoUnduck (cs2kz's 3rd real style) + a style-incompatibility check (ADOnly+WSOnly). Ice/LowGrav/WSOnly/ADOnly are our own extras (cs2kz never had them) — kept as bonus. |
| Timer | 🟡 PARTIAL | Timer engine + PRO/STANDARD + course switching. Start-gate has JustLanded + JustTeleported (0.05s each). Checkpoints purge on start-zone/disconnect. **Remaining (this session):** end-zone "you skipped a checkpoint/stage" gate; true pause-freeze. |
...
| Zones / Triggers | 🟡 PARTIAL | Mapping-API zones (Start/End/Stage/Checkpoint/Split) via keyvalue+origin correlation. Teleport + Multi/Single/Sequential-bhop + push (full condition set, delayed events, per-axis abs-speed, cooldown, cancel-on-tp) + anti-bhop + modifier (gravity/duck) all work. TriggerFix = per-tick live-hull overlap scan + path-swept bump replay (subtick dodging can't skip a trigger). **Remaining (this session):** reset-checkpoint + single-bhop-reset map-trigger runtime handlers (uncommon map feature). |
...
| HUD | 🟡 PARTIAL | One fixed HTML block. Remaining: perf-color tint, `h:mm:ss` formatting, compact mode, particle MHUD — genuinely minimal vs cs2kz's HUD. |
| Tip | 🟡 PARTIAL | Broadcast cycling works. Remaining: configurable interval, file-scan/shuffle loading. |
...
| Goto / Fov / Measure / Pistol / Noclip | 🟡 PARTIAL | Goto (timer-block + no-match now done; ladder/collision-reset minor), Fov, Pistol, Noclip work. Remaining: Measure aim-trace (measures own position; 3/4 subcmds), Pistol strip-enforce/team-aware, Noclip death-auto-disable safeguard. |
