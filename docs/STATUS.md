...
| Native movement detours | 🟡 | Full AirAccelerate→FinishMove surface hooked (18 sig detours + **FinishMove vtable hook**), ON by default. **Filled:** prestrafe, perf, **rampbug** (CategorizePosition), **AirMove maxspeed cap** — via the `{ms→slot}` map + `TraceShapePlayerMovement`. Detours for funcs CKZ doesn't override (AirAccelerate/Friction/WalkMove/CheckJump/Duck/…) are **correctly pass-through** (faithful). The one remaining fill: **TryPlayerMove** collision loop (high blast-radius — a wrong blind port breaks all movement; needs live iteration). All experimental, need demo validation. |
| Jumpstats | 🟡 | External `Kreedz.Jumpstats` plugin. **Basic** — LJ/BH + distance tiers only. Missing full stat set, jump-type classification, invalidation, jumpstats DB. |
