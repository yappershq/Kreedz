...
| Native movement detours | 🟡 | Hooked (sigs + typed `MoveData`), ON by default. **Filled:** prestrafe, perf, **rampbug** (CategorizePosition), **AirMove maxspeed cap** — via the `{ms→slot}` map + `TraceShapePlayerMovement`. Detours for funcs CKZ doesn't override (AirAccelerate/Friction/WalkMove/CheckJump/Duck/…) are **correctly pass-through** (engine behavior is faithful). Remaining: **TryPlayerMove** collision loop (high blast-radius — needs live iteration) + FinishMove vhook + SlopeFix/water/triggers. All experimental, need demo validation. |
